...
\noindent{\bf Problem 4}: Let $Y$ be a regular full exponential family with canonical parameter $\theta$. Verify that $Y$ is sub-exponential.

\vspace*{1cm}

\noindent{\bf Problem 5}: In the notes it was claimed that the scalar products of $\sum_{i=1}^n\{y_i - \nabla c(\theta)\}$ are also sub-exponential (page 15). Show that this is in fact true when the observations $y_i$ are iid from a regular full exponential family.
