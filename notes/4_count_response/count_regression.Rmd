...
We will suppose that we have a sample of data $(y_i,x_i)$, $i = 1,\ldots, n$ where $y_i$ is a scalar response variable and $x_i$ is a fixed vector of predictors taking values in $\R^p$. 

The likelihood of the exponential family is of the form
\begin{equation} \label{expolog}
	l(\theta) = \inner{y, \theta} - c(\theta),
\end{equation}
where $y$ is a vector statistic having components $y_i$ and $\theta \in \R^n$ is the canonical parameter vector. Without specifying a submodel, $\theta$ is unconstrained and the likelihood \eqref{expolog}, one parameter for every observation. We will specify a canonical linear submodel of an exponential family is a submodel having parameterization
$$
  \theta = M\beta
$$
where $\theta \in \R^n$ is the canonical parameter vector corresponding to the original saturated exponential family, $\beta \in \R^p$ is the canonical parameter vector for the submodel, and $M$ is the model matrix with rows $x_i^T$. The submodel log likelihood is 
...
```{r, out.width = '50%', fig.align="center", echo = FALSE, fig.cap="A depiction of the transformations necessary to change between parameterizations. Arrows going in opposite directions specify transformations and their inverses. M is a known model matrix of full column rank, and c is the cumulant function for the exponential family model." }
...
\vspace*{0.5cm}\noindent{\bf Manually write your own Fisher scoring algorithm which maximizes the Poisson regression log likelihood for this example. Report $\hat\beta$ and reproduce the above summary table without using the glm or summary commands.}
...
$$
where $0 < \alpha < 1$ is some error threshold. The $(1 - \alpha)\times 100\%$ Wald confidence interval for the log response variable $\log(\mu(x))$ at a particular $x$ follows from the Delta method. We have that $\log(\mu(x)) = x^T\beta$ which implies that $\nabla_{\beta}\log(\mu(x)) = x$, and we have that 
$$
  \sqrt{n}(\hat\beta - \beta) \overset{d}{\to} N(0, \Sigma^{-1})
$$
Therefore,
$$
  \sqrt{n}\left(\log(\hat\mu(x)) - \log(\mu(x))\right) \overset{d}{\to} N(0, x^T\Sigma^{-1}x).
$$
We can report the following $(1 - \alpha)\times 100\%$ Wald confidence interval for the log mean value
$$
  \log(\hat\mu(x)) \pm z_{\alpha/2}\sqrt{x^T\widehat\Sigma^{-1}x}.
$$
...
\vspace*{0.5cm}\noindent{\bf Summarize the summary table produced by a call to summary(m1).}
...
As with standard linear models, it is important to check the modeling assumptions of the GLM. The diagnostic methods are similar to those used for linear models with normal errors. However, some adaptions are necessary, and not all diagnostic methods will be applicable.
...
\vspace*{0.5cm}\noindent A [half-normal plot](https://www.stat.cmu.edu/technometrics/59-69/VOL-01-04/v0104311.pdf) of the residuals of the Poisson model is shown on the left and the relationship between the mean and the variance is shown on the right. A line showing that the variance increases linearly in the mean (not a perfect slope of 1) is also shown.
```{r}
par(mfrow = c(1,2))
...
We notice that the \texttt{ment} variable which counts the number of articles produced by the mentor is the most significant predictor in both the Poisson and binary regressions. This is because the zero-inflated approach models the probability of a zero count (not the probability of a successful 1 count). Hence there is no contradiction.
...
```{r loaddata2, cache = TRUE, echo = FALSE, eval = FALSE}
library(faraway)
m2 <- glm(Species ~., family = "poisson", data = gala)
m2 <- glm(Species ~ Area + Elevation + Nearest + Scruz + Adjacent, 
...
CCSO_small2 <- CCSO2 %>% rename(Days = "Days in Jail", Age = "Age at Arrest", 
  ...
  dplyr::select(Days, Age, Sex, Date, Race, Crime, Agency) %>%
...
m1 <- glm(Days ~ Race + Sex + Age, family = "poisson", data = CCSO_small2)
summary(m1)

Ey <- predict(m1, type = "response")

plot(Ey, (y - Ey) / sqrt(Ey), pch = 19)


m2 <- glm(Days ~ Race + Sex + Age + Agency, family = "poisson", data = CCSO_small2)
summary(m2)

m3 <- glm(Days ~ Race + Sex + Age + Agency, family = "quasipoisson", data = CCSO_small2)
summary(m3)

par(mfrow = c(1,2))
plot(predict(m2, type = "link"), residuals(m2), 
     xlab = "theta", ylab = "Deviance residuals", pch = 19)
plot(predict(m3, type = "link"), residuals(m3), 
     xlab = "theta", ylab = "Deviance residuals", pch = 19)

plot(predict(m2, type = "link"), residuals(m2, "pearson"), 
     xlab = "theta", ylab = "Pearson residuals", pch = 19)
plot(predict(m3, type = "link"), residuals(m3, "pearson"), 
     xlab = "theta", ylab = "Pearson residuals", pch = 19)
```
