...
$$
where $\mu$ a population mean fixed effect, $\tau_i$ is a sub population fixed-effect, $v_j$ is a random effect, and $\varepsilon_{ijk}$ is an error term. We will assume that $v_j \overset{iid}{\sim} N(0,\sigma^2_v)$ and $\varepsilon_{ijk} \overset{iid}{\sim} N(0,\sigma^2)$. In this model we want to estimate $\tau_i$ and test the hypothesis $H_0: \tau_i = 0$ while we would estimate $\sigma^2_v$ and test $H_0:\sigma^2_v = 0$. Notice that we want to estimate and test all of the fixed-effects $\tau_i$ while we only need to estimate and test a single random effects parameter.
...
$$
\textbf{Prove that the above is true, and discuss the implications of a value of $\rho$ that is close to zero or one.} 
...
$$
where $Y \in \R^n$ is the response vector, $X \in \R^{n\times p}$ is a fixed model matrix, $\beta \in \R^p$ is a coefficient vector and is considered a fixed effect, $Z \in \R^{n\times q}$ is a fixed model matrix for the random effects, $b \in \R^q$ is a vector of random effects, and $\sigma^2$ is the variance of the error distribution. If we further assume that the random effects satisfy $b \sim N(0, \sigma^2D)$, then the unconditional response $Y$ has the distribution
$$
...
$$
  l(\beta,\sigma,D|y) \propto \frac{\log|\sigma^2V|}{2} + \frac{(Y-X\beta)^TV^{-1}(Y-X\beta)}{2}.
$$
...
The default fitting method is the REML procedure mentioned above. We see that this method gives identical estimates to the ANOVA results above. For unbalanced designs, the REML and ANOVA estimators are not necessarily identical. \textbf{Analyze this data using unrestricted maximum likelihood estimation and comment on the differences that such analysis produces when compared to the REML and ANOVA estimates.}
...
Note that we may use bootstrap methods to find more accurate $p$-values for the LRT. The most popular bootstrap approach is nonparametric in that no distributional assumptions are made. Since we are willing to assume normality for the errors and the random effects in our modeling formulation, we can use a technique called the \textbf{parametric bootstrap}. In this technique, we generate data under the null model using the fitted parameter estimates as guesses for the true parameter values. We compute the LRT test statistic for this generated data. We repeat this scheme many times and compare the observed LRT test statistic to the distribution of LRT test statistics obtained from repeatedly generation of samples. We will discuss this approach below. \textbf{Compare and contrast the nonparametric bootstrap, residual bootstrap, and parametric bootstraps. Discuss the assumptions that make each bootstrap procedure appropriate.} [Hint: Here is a useful reference.](https://www.stat.cmu.edu/~cshalizi/uADA/13/lectures/which-bootstrap-when.pdf)
...
sqrt(pval*(1-pval)/B)
```
We can be fairly sure that the estimated $p$-value is under a 0.05 nominal level. If in doubt, do some more replications to make sure; this only costs computer time (\textbf{Perform the above analysis with $B = 1e4$ and make it as fast as possible using parallel programming with either \texttt{mclapply} and/or \texttt{foreach} and whatever accompanying software packages are needed.}) As it happens, this $p$-value is close to the fixed effects $p$-value. 
...
\vspace*{0.5cm}\textbf{Interpret these plots and conclude that these plots indicate no particular problems.}
...
The researchers are interested in determining which genotypes are associated with a different expression of apparent quantum efficiency (AqE) from the RC reference genotype. AqE is a part of the photosynthetic process in plants where improvements in AqE allow for improvements in yield.
...
This data consists of multiple measurements on the same day with only a few days of data collection. Here is a depiction of the relationships between the response and the covariates.

```{r}
pairs(dat[,4:7])
```

We now investigate the distribution of signal to noise ratios ($\bar{x}/\text{sd}(x)$) for the plot and disk effects. We can see that a significant portion of the range of AqE is explained by variation in plots. A smaller portion is explained by variation in disk (measurement miscalibration).
...
## signal-to-noise for plot effects
sig2noise_plot <- sapply(unique(dat$plot_number), function(x) mean(dat[dat$plot_number == x, ]$AqE) / 
                   sd(dat[dat$plot_number == x, ]$AqE))
hist(sig2noise_plot, breaks = 20, main = "Histogram of signal-to-noise ratios for plot effect")
```


```{r}
## signal-to-noise for disk effects
disk_num <- sort(unique(dat$disk))
sig2noise_disk <- sapply(disk_num, function(x) mean(dat[dat$disk == x, ]$AqE) / 
                   sd(dat[dat$disk == x, ]$AqE))
...
system.time(for(b in 1:B){
  y <- unlist(simulate(m))
  bnull <- lm(y ~ ID + days + I(days^2) + Precip + Precip_7day, 
    data = dat)
  balt <- lmer(y ~ ID + days + I(days^2) + Precip + 
      Precip_7day + (1|plot_number), data = dat, REML = FALSE)
  lrtstat[b] <- as.numeric(2*(logLik(balt) - logLik(bnull)))
})
```
...
```{r}
mean(lrtstat < 1e-5)
lrtstat[lrtstat > 1e-5]
```
The bootstrapped p-value is 0.
```{r}
## p-value
pval <- mean(lrtstat > as.numeric(2*(logLik(m_re_plot) - logLik(m))))
pval
...
system.time(for(b in 1:B){
  ...
  bnull <- lmer(y ~ ID + days + I(days^2) + Precip + 
      Precip_7day + (1|plot_number), data = dat, REML = FALSE)
  balt <- lmer(y ~ ID + days + I(days^2) + Precip + 
      Precip_7day + (1|plot_number) + (1|disk), data = dat, REML = FALSE)
  lrtstat[b] <- as.numeric(2*(logLik(balt) - logLik(bnull)))
...
```{r}
## p-value
pval <- mean(lrtstat > as.numeric(2*(logLik(m_re_full) - logLik(m_re_plot))))
pval
...
M <- model.matrix(AqE ~ ID + days + I(days^2) + Precip + Precip_7day, 
                  data = dat)
...
<!-- ```{r} -->
<!-- dotplot(ranef(m_re_full, condVar=TRUE)) -->
<!-- ``` -->
...
exactRLRT(lmod)
```
\textbf{Explain how the above testing procedure works.} We see that the fields do seem to vary as the result is clearly significant.
...
## R example Panel Study of Income Dynamics
...
library(tidyverse)
data(psid)
psid <- mutate(psid, gender = sex) %>% dplyr::select(-sex)
head(psid)
```
...
ggplot(psid20, aes(x=year, y=income)) + 
  ...
  theme_minimal()
```
We see that some individuals have a slowly increasing income, typical of someone in steady employment in the same job. Other individuals have more erratic incomes. We can also show how the incomes vary by gender. Income is more naturally considered on a log-scale:
```{r}
...
pgender <- psid$gender[match(1:85,psid$person)]
...
mmod <- lmer(log(income) ~ cyear*gender + age + educ + (cyear|person), psid)
```
This model can be written as:
$$
  \log(\text{income})_{ij} = \mu + \beta_y\text{year}_j + \beta_g \text{gender}_i + \beta_{yg}\text{gender}_i*\text{year}_j
...
However, a mixed-model formula incorporates $k \geq 1$ random-effects terms as well. These $k$ terms are used to produce the random-effects model matrix $Z$, and the structure of the relative covariance factor $\Lambda_\theta$, which are matrices that typically have a sparse structure. 

We now describe how one might construct these matrices from the random-effects terms, considering first a single term $(r|f)$ and then generalizing to multiple terms. The above tables summarize the matrices and vectors that determine the structure of $Z$ and $\Lambda_\theta$. 
...
\textbf{Examples}: The PSID example above considered $k=1$ term \texttt{(cyear|person)} which corresponded to a random intercept and a random slope for an individual's income over time. In this example $X_i, i=k=1$, corresponds to a model matrix consisting of an intercept and a column for years centered around 1978, the median value, and $J_i$ provides indexes for each individual in the study. The Bliss (1967) example above concerning consistency of measurements considered $k=3$ terms \texttt{(1|Lab)},\texttt{(1|Lab:Technician)}, and \texttt{(1|Lab:Technician:Sample)}. Here, $X_i, i=1,2,3,$ correspond to intercept vectors and the $J_i$ matrices consider separate grouping factors which reflect the nested random effects.
...
$$
\textbf{Homework: Derive the above}. The above derivation is an important expression in the underlying \texttt{lme4}. It relates the penalized residuals sum-of-squares with its minimum value and is extremely useful when we integrate out the random effects, which is required for maximum likelihood estimation. Note that we have simplified notation by writing $r^2(\theta,\hat\beta_\theta,\mu_{U|Y=\yobs})$ as $r^2(\theta)$.


## Probability densities

The residual sums-of-squares discussed in the previous section can be used to express various probability densities, which are required for maximum likelihood estimation of the LMM,
\begin{align*}
...
\noindent We borrow materials from \cite{faraway2016extending}, \cite{pinheiro2006mixed}, \cite{agresti2013cat}, the [vignette](https://cran.r-project.org/web/packages/lme4/vignettes/lmer.pdf) for the \texttt{lem4} package, and Adam Rothman's notes on computing that are included along with these notes.
