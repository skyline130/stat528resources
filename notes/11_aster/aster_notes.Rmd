...
Aster models are simple graphical model in which  the joint density is a product of conditionals as in equation \eqref{eq:factor} below. Variables in an aster model are denoted by $X_j$, where $j$ runs over the nodes of a graph. These graphical models follow six assumptions which are: 

- \textbf{A1} The graph of arrows is acyclic.

- \textbf{A2} In the graph of lines every connected component is a complete graph, which is called a dependence group. 
...
We take each of the conditional distributions in \eqref{eq:factor} to be an exponential family having canonical statistic $X_G$ that is the sum of $X_{p(G)}$ independent and identically  distributed random vectors, possibly a different such family for each $G$. Conditionally, $X_{p(G)}=0$ implies that $X_G=0_G$ almost surely. The log likelihood for the whole family has the form
...
$$
  \sqrt{n}(g(\hat\beta) - g(\beta)) \to N\left(0, \nabla g(\beta)\Sigma^{-1} \{\nabla g(\beta)\}^T \right).
$$
...
The result of \texttt{predict(aout)} is the maximum likelihood estimate of the saturated model mean value parameter vector $\mu$. If we want to say how bad or good our estimators are, then weneed confidence intervals (or perhaps just standard errors).
