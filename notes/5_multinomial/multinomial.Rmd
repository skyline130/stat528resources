...
\end{itemize}
\vspace*{0.5cm}\noindent Let categories be coded as $1,...,J$ (in order, if ordinal). In these notes we model the distribution of $Y$ in terms of explanatory variables $X_1,..., X_p$. Let 
$$
  \pi_j(x) = \Prob\left(Y \; \text{in category} \; j\mid x\right), \qquad j = 1,...,J,
$$
where $x \in \R^p$, and note that the multinomial model has the constraint
$$
...
We consider MLEs $\hat{\beta}_1,\ldots,\hat{\beta}_{J-1}$ when they uniquely exist. When would the MLEs not uniquely exist? Note that the total number of (scalar) free parameters is $(J-1)p$ where $p$ is the number of explanatory variables $(\beta_j \in \R^p)$. Substituting the MLEs gives estimated response functions 
$$
...
\textbf{Question}: Fit the \texttt{nnet} model and comment on the similarities and differences between the \texttt{nnet} and \texttt{VGAM} fits. Report interesting conclusions using either implementation.
...
\end{array}\right).
$$
So happiness is estimated lower ($Y$ is estimated to be larger) as trauma increases. For example, we can estimate the odds of ``very happy'' for control category A relative to control category B (trauma held fixed), and a Wald 95\% confidence interval for these estimates:
...
Note: Signs of coefficients (but not intercepts) are different from what was obtained using \texttt{vglm}, because \texttt{polr} uses a different parameterization (See Section 8.2.3 in \cite{agresti2013cat}). \textbf{Comment on the differences between the \texttt{vglm} and \texttt{polr} implementations.}
...
\noindent These notes closely follow Trevor Park's slides. We also borrow materials from \cite{agresti2013cat}.
