...
\vspace{12pt}
We will now investigate classification using a confusion matrix on the test data where we encode a success when $\hat{p} > 0.5$, and a failure when $\hat{p} \leq 0.5$. From the output below, we see that classification accuracy of 0.8439 is much higher than the no information rate of 0.7081. Importantly, the [sensitivity and specificity](https://en.wikipedia.org/wiki/Sensitivity_and_specificity) are, respectively, 0.9023 and 0.8198.
